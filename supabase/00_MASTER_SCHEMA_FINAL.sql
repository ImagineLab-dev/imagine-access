...
BEGIN
  ...
  LEFT JOIN public.users_profile up ON t.created_by = up.user_id
  WHERE
    e.organization_id = v_my_org -- Strict Multi-Tenant Enforcement Layer
    AND (
      EXISTS (
...
BEGIN
  ...
  IF v_caller_role <> 'admin' THEN
      RAISE EXCEPTION 'Cannot delete: Only Admins can delete users';
  END IF;

  -- Delete from public tables first (cascade handles some of it, but just to be sure)
  DELETE FROM public.users_profile WHERE user_id = p_target_id AND (role != 'admin' OR user_id = auth.uid());
  
  -- The actual auth.users deletion needs to be handled via Edge Function (Supabase Admin),
  -- but we can clear their profile so they lose all access immediately.
  RETURN true;
END;
$$;
...
AFTER UPDATE OF role, organization_id ON public.users_profile
...
FOR UPDATE USING (
  -- Either the admin of the corp
  (COALESCE(auth.jwt() -> 'app_metadata' ->> 'role', 'rrpp') = 'admin' AND organization_id = public.get_my_organization_id())
  OR 
